...
void heap_caps_init(void)
{
...
#ifdef RIOT_VERSION
        ESP_EARLY_LOGI("heap_init", "At %08x len %08x (%d KiB): %s",
                       g_heap_region[heap_region_num].start_addr,
                       g_heap_region[heap_region_num].total_size,
                       g_heap_region[heap_region_num].total_size / 1024, "IRAM");
#endif
        heap_region_num++;
    }
#endif
    ...
    ESP_EARLY_LOGI("heap_init", "At %08x len %08x (%d KiB): %s",
                   g_heap_region[heap_region_num].start_addr,
                   g_heap_region[heap_region_num].total_size,
                   g_heap_region[heap_region_num].total_size / 1024, "DRAM");
#endif
    heap_region_num++;
