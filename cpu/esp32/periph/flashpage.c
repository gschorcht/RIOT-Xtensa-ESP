...
#if ENABLE_DEBUG
FLASH_WRITABLE_INIT(_fp_test_array, 1);     /* test array with 1 page */
#endif

void esp_sync_cache(uint32_t target_addr, size_t len)
{
...
void IRAM_ATTR esp_flashpage_init(void)
{
    ...
    if (ENABLE_DEBUG) {
        spi_flash_mmap_dump();
        DEBUG("%s _fp_flash_start @%p\n", __func__, _fp_test_array);
    }
